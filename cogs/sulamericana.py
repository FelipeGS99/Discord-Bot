...
class Sulamericana(commands.Cog):
    def __init__(self, bot: commands.Bot) -> None:
        self.bot = bot
        state_path = Path(__file__).resolve().parent.parent / "sulamericana_state.json"
        ...
        self.league_id = settings.sulamericana_league_id
    ...
    async def cog_unload(self) -> None:
        self.check_sulamericana_scores.cancel()

    @commands.group(name="sulamericana", invoke_without_command=True)
    async def sulamericana_group(self, ctx: commands.Context) -> None:
        prefix = ctx.prefix or "?"
        await ctx.send(
            "\n".join(
                [
                    "**Sul-Americana**",
                    "Use para consultar jogos e configurar alertas automÃ¡ticos da Sul-Americana.",
                    f"`{prefix}sulamericana hoje` - Mostra jogos de hoje com horÃ¡rio, placar, status e gols quando disponÃ­vel.",
                    f"`{prefix}sulamericana canal #canal` - Ativa alertas de inÃ­cio, gol, intervalo, volta de status e fim de jogo. Exemplo: `{prefix}sulamericana canal #placares`.",
                    f"`{prefix}sulamericana status` - Mostra canal configurado, API, liga e cache atual.",
                    f"`{prefix}sulamericana parar` - Desativa os alertas automÃ¡ticos da Sul-Americana.",
                ]
            )
        )

    @sulamericana_group.command(name="canal")
    async def set_channel(self, ctx: commands.Context, channel: discord.TextChannel) -> None:
        if not self._can_manage_scores(ctx):
            await ctx.send("Voce nao tem permissao para configurar os placares da Sul-Americana.")
            return
        ...
        await ctx.send(
            f"Placares da Sul-Americana ativados em {channel.mention}. "
            "Vou avisar quando houver mudanca de status ou placar."
        )

    @sulamericana_group.command(name="hoje")
    async def today(self, ctx: commands.Context) -> None:
        ...
        if not fixtures:
            await ctx.send("Nao encontrei jogos da Sul-Americana para hoje.")
            return
    ...
    @sulamericana_group.command(name="status")
    ...
    @sulamericana_group.command(name="parar")
    async def stop_scores(self, ctx: commands.Context) -> None:
        ...
        self._save_state()
        await ctx.send("Placares da Sul-Americana desativados neste canal.")

    @set_channel.error
    async def set_channel_error(self, ctx: commands.Context, error: commands.CommandError) -> None:
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send("Informe o canal. Exemplo: `sulamericana canal #placares`.")
            return
    ...
    async def check_sulamericana_scores(self) -> None:
        ...
        try:
            ...
            print(f"Erro ao atualizar placares ao vivo da Sul-Americana: {exc}")
            return
        ...
        if not fixtures:
            ...
            try:
                ...
                print(f"Erro ao consultar placares da Sul-Americana: {exc}")
                return
        ...
        for fixture in changed_fixtures:
            try:
                ...
                print("Sem permissao para enviar placares da Sul-Americana.")
                return
    ...
    async def _send_score_to_channels(self, embed: discord.Embed) -> None:
        for channel_id in self.channel_ids:
            ...
            if not isinstance(channel, discord.TextChannel):
                print(f"Canal de placares da Sul-Americana nao encontrado: {channel_id}")
                continue
            try:
    ...
    @check_sulamericana_scores.before_loop
    async def before_check_sulamericana_scores(self) -> None:
    ...
    async def _fetch_goal_scorers(self, fixture: BrasileiraoFixture) -> list[str]:
        ...
        try:
            ...
            print(f"Erro ao buscar autores dos gols da Sul-Americana: {exc}")
            return []
    ...
    def _build_today_embed(
        ...
        embed = discord.Embed(title="Sul-Americana - jogos de hoje", color=EMBED_COLOR)
        for fixture in fixtures:
...
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(Sulamericana(bot))
